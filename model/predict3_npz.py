...
def read_event_npz(name, fps=1000):
    arr = np.load(name, allow_pickle=True)['events']
...
class AedatDataset(Dataset):
    ...
    def load(self, name):
        self.events = read_event_npz(name)
    ...
    def __getitem__(self, idx):
        idx = idx - self.WINDOW + 1
        img = np.zeros((HEIGHT, WIDTH, 2), np.float32)
        for i in range(self.WINDOW):
            if idx+i<0:
                continue
            if idx+i>=len(self.events):
                break
            if len(self.events[idx+i]) > 0:
                xs, ys, ps = self.events[idx+i].T
                img[ys, xs, ps] = i/self.WINDOW
...
if __name__ == '__main__':
    ...
    with open(sys.argv[3], 'w') as prf:
        ...
        for x, bidx in zip(test_loader, tqdm(range((len(testconcat))//test_loader.batch_size))):
