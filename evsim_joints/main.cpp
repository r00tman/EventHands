...
//    glm::vec3 eyePos = glm::vec3(0.0f, 0.0f, 2.0f);
    ...
    // if we want to output the joint positions, write the virtual camera parameters file
    if (JOINT_OUTPUT.length() > 0) {
        std::string camerafn = JOINT_OUTPUT;
        if (camerafn.length() >= 4 && camerafn.substr(camerafn.length()-4, 4) == ".txt") {
            camerafn = camerafn.substr(0, camerafn.length()-4);
        }
        camerafn = camerafn + "_camera.txt";
        FILE *matFile = fopen(camerafn.c_str(), "w");
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j)
            fprintf(matFile, "%f ", proj[j][i]);
        }
        fprintf(matFile, "\n");
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j)
            fprintf(matFile, "%f ", view[j][i]);
        }
        fprintf(matFile, "\n");
        fflush(matFile);
        fclose(matFile);
...
//    screenRender = false;
    ...
    while (true) {
        ...
        if (changed) {
            ...
            // TODO: normalize by .w!!!!!!!!! this is NDC, perspective is done via .w
...
//            writer->write_poses(xt::xarray<double>{m.get_trans().x, m.get_trans().y, m.get_trans().z});
        ...
        if (JOINT_OUTPUT.length() > 0) {
            ...
            for (int i = 0; i < jtr.shape(0); ++i) {
                glm::vec3 pos{jtr.at(i, 0), jtr.at(i, 1), jtr.at(i, 2)};
//                pos = m.get_hand_position();
                pos = trans * glm::vec4(pos, 1.0);
                fprintf(jointFile, "%f %f %f ", pos.x, pos.y, pos.z);
            }
            fprintf(jointFile, "\n");
            fflush(jointFile);
        }
    ...
    //        newtrans = glm::translate(newtrans, glm::vec3(-0.45f, -0.05f, -0.1f));
//            newtrans = glm::translate(newtrans, m.get_hand_position()-m.get_trans());
            xt::xarray<double> jtr = m.get_joints();
            int idx = 21;
            idx = 37;
            idx = 52;
            newtrans = glm::translate(newtrans, glm::vec3(jtr(idx, 0), jtr(idx, 1), jtr(idx, 2)));
            glm::vec3 rot = m.get_hand_rotation();
            newtrans = glm::rotate(newtrans, glm::length(rot), glm::normalize(rot));
            newtrans = glm::rotate(newtrans, glm::radians(-90.0f), glm::vec3(1.0, 0.0, 0.0));
    //        newtrans = glm::scale(newtrans, glm::vec3(0.15, 0.15, 0.15));
//            newtrans = glm::scale(newtrans, glm::vec3(0.02, 0.02, 0.02));
//            newtrans = glm::scale(newtrans, glm::vec3(0.01, 0.01, 0.01));
            newtrans = glm::scale(newtrans, glm::vec3(0.005, 0.005, 0.005));
    //        newtrans = m.transform * newtrans;
    //        newtrans = glm::translate(newtrans, glm::vec3(-0.45f, -0.05f, 0.4f));
            ...
            glUniformMatrix4fv(auxShader["model"], 1, GL_FALSE, glm::value_ptr(newtrans));
