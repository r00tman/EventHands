...
// ids of fingertips
const int fids[] = {6207, 5782, 5906, 6017, 6134};
...
void Mano::ready_arguments(bool wantVPosed) {
    ...
    if (wantShapeModel) {
        ...
        if (betasChanged || poseChanged) {
            ...
            dd.fingertipsPosed = xt::zeros<double>({5UL, 3UL});
            for (int i = 0; i < 5; ++i) {
                xt::xarray<double> fpos;
                fpos = xt::view(dd.vShaped, fids[i]) + xt::linalg::dot(xt::view(dd.poseDirs, fids[i]), dd.posemapped);
                xt::view(dd.fingertipsPosed, i) = fpos;
            }
        }
    } else {
...
xt::xarray<double> Mano::get_joints() {
    ...
    xt::xarray<double> fullwgt, wgt, result;
    fullwgt = dd.weights.reshape({6890UL, 52UL});
    wgt = xt::zeros<double>({5UL, 52UL});
    for (size_t i = 0; i < 5; ++i) {
        xt::view(wgt, i) = xt::view(fullwgt, fids[i]);
    }

    auto T = xt::transpose(xt::linalg::dot(xt::view(dd.A, xt::range(_, 3)), xt::transpose(wgt)));

    auto rest_shape_h = xt::vstack(xt::xtuple(xt::transpose(dd.fingertipsPosed),
                                              xt::ones<double>({1UL, dd.fingertipsPosed.shape(0)})));

    xt::xarray<double> v = 0;

    for (unsigned long i = 0; i < 4; ++i) {
        auto ct = xt::view(T, xt::all(), i, xt::all());
        xt::xarray<double> cv = xt::view(rest_shape_h, i, xt::all(), xt::newaxis());
        auto tmp = ct * cv;
        v += tmp;
    }

    dd.fingertipsSkinned = v;

    result = xt::vstack(xt::xtuple(dd.Jtr, dd.fingertipsSkinned));
