...
def compute_event_stream_errors(gt_list, pred_list, shift=0.):
    ...
    for seq_id in range(len(gt_list)):
        ...
        print('loading')
        tstart = time.time()
        gt = np.loadtxt(gt_file, max_rows=10000000)
        print('loaded gt')
        pred = np.loadtxt(predictions_file, max_rows=10000000)
        print('loaded pred')
        print('loaded in', time.time()-tstart, 's')

        # gt_timestamp = gt[:,0]
        # pred_timestamp = pred[:,0]
        # gt_dict = {gt_timestamp[i]: gt[i,1:].reshape(-1,2) for i in range(gt.shape[0])}
        for i in trange(len(pred)):
            ...
            gt_values = gt[i+shift, 1:].reshape(-1, 2) if i+shift<len(gt) and i+shift>=0 else None
...
if __name__ == "__main__":
    gt_files = ['./synthjoints/synthdemo_joints_ss.txt']
    ...
    plot_PCK(fname, errors_list, labels, color_list, title="Avg. Keypoint Error 2D", xrange=[0, 100], showAUC=True)
